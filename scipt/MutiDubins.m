...
%输出量为协同航迹(航点+航向)(5*3*n)【n为飞行器数量，列包含点位X坐标、Y坐标和航向角】
%优点：起始和终止航向角可控，直接使用真实坐标无需转换；缺点：暂无避撞策略且航迹单一
function result =  MutiDubins(Start, Goal, Velocity, PhiMaximum)
    rad2deg = 57.295779513082320876798154814105;
    %初始化
    temp=struct('xs',0, ...%起点x坐标
                  'ys',0, ...%起点y坐标
                  'psi_s',0, ...%起始航向角
                  'xf',0, ...%终点x坐标
                  'yf',0, ...%终点y坐标
    ...
    for i=1:length(object)
        [object(i).xts,object(i).yts,object(i).xtf,object(i).ytf,object(i).l,object(i).pos,object(i).pof,object(i).r,object(i).index_dubins(1),object(i).index_dubins(2)] ...
        ...
        if(i~=target)
            ...
            for j=1:precision
                while(stop_flag)
                    ...
                    [object(i).xts,object(i).yts,object(i).xtf,object(i).ytf,object(i).l,object(i).pos,object(i).pof,~,object(i).index_dubins(1),object(i).index_dubins(2)] ...
    ...
    %结果输出
    result=zeros(5,3,length(object));
    for i=1:size(result,3)
        result(1,:,i)=[object(i).xs,object(i).ys,object(i).psi_s];
        result(2,:,i)=[object(i).xts(object(i).index_dubins(1),object(i).index_dubins(2)),object(i).yts(object(i).index_dubins(1),object(i).index_dubins(2)),deltapsi(object(i).xtf(object(i).index_dubins(1),object(i).index_dubins(2)),object(i).ytf(object(i).index_dubins(1),object(i).index_dubins(2)),object(i).xts(object(i).index_dubins(1),object(i).index_dubins(2)),object(i).yts(object(i).index_dubins(1),object(i).index_dubins(2)))*rad2deg];
        result(3,:,i)=[object(i).xtf(object(i).index_dubins(1),object(i).index_dubins(2)),object(i).ytf(object(i).index_dubins(1),object(i).index_dubins(2)),deltapsi(object(i).xtf(object(i).index_dubins(1),object(i).index_dubins(2)),object(i).ytf(object(i).index_dubins(1),object(i).index_dubins(2)),object(i).xts(object(i).index_dubins(1),object(i).index_dubins(2)),object(i).yts(object(i).index_dubins(1),object(i).index_dubins(2)))*rad2deg];
        result(4,:,i)=[object(i).xm,object(i).ym,object(i).psi_f];
        result(5,:,i)=[object(i).xf,object(i).yf,object(i).psi_f];
    end
end
